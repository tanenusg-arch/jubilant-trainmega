...
async function main() {
  ...
  // Render provides RENDER_EXTERNAL_URL automatically. You can also set
  // WEBHOOK_URL manually (for example: https://your-service.onrender.com).
  const PORT = Number(process.env.PORT || 10000);
  const BASE_WEBHOOK_URL = String(
    process.env.WEBHOOK_URL || process.env.RENDER_EXTERNAL_URL || ""
  ).replace(/\/$/, "");
  if (!BASE_WEBHOOK_URL) {
    throw new Error("WEBHOOK_URL or RENDER_EXTERNAL_URL is required for Render webhook mode.");
  }

  const WEBHOOK_PATH = String(process.env.WEBHOOK_PATH || "/telegram/webhook").replace(/^([^/])/, "/$1");
  ...
  const healthServer = http.createServer((req, res) => {
    if (req.method === "GET" && (req.url === "/" || req.url === "/health")) {
      ...
      res.end("YABTOPUP bot is running in Render webhook mode.");
      return;
    }
  ...
  healthServer.on("error", (e) => logger.error(`Webhook HTTP server error: ${e.message}`));
  ...
  logger.info(`Webhook HTTP server listening on port ${PORT}`);

  // Schedule pending-payment cleanup while the webhook server is running.
  ...
  // Register this Render service as Telegram's webhook endpoint.
  await bot.telegram.setWebhook(WEBHOOK_URL);
  logger.info(`Telegram webhook registered: ${WEBHOOK_URL}`);
  logger.info("Telegram bot is ready in Render webhook mode.");
}
