...
class G2BulkAPIClient {
  ...
  async place_order(game_code, package_id, player_id, server_id = null, user_ip = null) {
    ...
    const catalogues = cat?.catalogues || cat?.data || [];
    if (Array.isArray(catalogues)) {
      const pkg = catalogues.find(p =>
        String(p.id ?? p.code ?? p.name ?? p.catalogue_name) === String(package_id)
      );
      if (pkg) {
        catalogue_name = pkg.name || pkg.catalogue_name || pkg.display_name || pkg.title || package_id;
      }
    }
    const payload = { catalogue_name: String(catalogue_name), player_id: String(player_id) };
    if (server_id) payload.server_id = String(server_id);
    if (user_ip) payload.user_ip = String(user_ip);
    logger.info(`[G2BULK] Creating game order | game=${game_code} | catalogue=${payload.catalogue_name} | player=${payload.player_id}`);
    const result = await this._request(`/games/${encodeURIComponent(game_code)}/order`, "POST", payload);
    logger.info(`[G2BULK] Game order response | ${JSON.stringify(result)}`);
    return result;
  }

  async get_game_order_status(order_id) {
    if (!order_id) return { success: false, message: "Missing G2Bulk order ID" };
    return await this._request("/games/order/status", "POST", { order_id: order_id });
  }
}
...
async function place_order_flow(ctx, session, db, api_client, bot, method, reference = null, verified_amount = null) {
  ...
  // Charge the customer only before the provider request. If G2Bulk rejects
  // the order, the provider did not consume its balance and we refund the user.
  // Once G2Bulk accepts a game order, its own wallet is charged by G2Bulk.
  if (method === "wallet") {
  ...
  try {
    if (session.data.flow_type === "voucher") {
      ...
      providerAccepted = !!(api_res && (api_res.success === true || api_res.ok === true) && api_res.order_id);
    } else {
      api_res = await api_client.place_order(game_code, package_id, player_id, server_id, "127.0.0.1");
      // G2Bulk game orders normally return success=true and an order object.
      // Do not treat the local bot order as delivered until the provider says
      // COMPLETED. PENDING/PROCESSING means the provider has accepted and is
      // still delivering, so its balance has already been charged.
      providerAccepted = !!(api_res && (
        api_res.order?.order_id || api_res.order_id ||
        api_res.status === "PENDING" || api_res.status === "PROCESSING"
      ) && (api_res.success !== false));
    }

    if (!providerAccepted) {
      const err = api_res?.message || api_res?.error || "G2Bulk did not accept the order.";
      await db.update_balance(user.id, charged_price);
      const orderId = session.data.pending_order_id || await db.create_order(
        user.id,
    ...
    let providerOrderId = api_res?.order?.order_id || api_res?.order_id || null;

    // Game orders are asynchronous. Poll G2Bulk until COMPLETED or FAILED so
    // every PUBG package is actually delivered before the bot says success.
    if (session.data.flow_type === "pubg" || session.data.flow_type === "game") {
      providerStatus = String(api_res?.order?.status || api_res?.status || "PENDING").toUpperCase();
      if (providerOrderId && !["COMPLETED", "FAILED"].includes(providerStatus)) {
        for (let attempt = 1; attempt <= 20; attempt++) {
          await new Promise(resolve => setTimeout(resolve, 3000));
          const statusRes = await api_client.get_game_order_status(providerOrderId);
          logger.info(`[G2BULK] Poll ${attempt}/20 | order=${providerOrderId} | ${JSON.stringify(statusRes)}`);
          const statusObj = statusRes?.order || statusRes?.data || statusRes || {};
          providerStatus = String(statusObj.status || statusRes?.status || "PENDING").toUpperCase();
          if (["COMPLETED", "FAILED"].includes(providerStatus)) {
            api_res = { ...api_res, ...statusRes, order: { ...(api_res.order || {}), ...statusObj } };
            break;
          }
        }
      }
    } else {
      providerStatus = String(api_res?.status || "COMPLETED").toUpperCase();
    }

    const success = providerStatus === "COMPLETED" || providerStatus === "SUCCESS";
    const failed = providerStatus === "FAILED" || providerStatus === "CANCELED" || providerStatus === "CANCELLED";
    ...
    if (orderId) {
      await db.update_order(orderId, {
        ...
        api_price,
        charged_price,
        status: success ? "completed" : (failed ? "failed" : "processing"),
        reference: reference || "",
      ...
      orderId = await db.create_order(
        ...
        api_price,
        charged_price,
        success ? "completed" : (failed ? "failed" : "processing"),
        reference,
      );
      await db.update_order(orderId, { provider_order_id: providerOrderId || "", provider_status: providerStatus });
    }
    ...
    if (success) {
      const escapeHtml = (value) => String(value ?? "")
        ...
        .replace(/>/g, "&gt;")
        .replace(/"/g, "&quot;");
      const deliveryText = api_res?.delivery_items || api_res?.order?.delivery_items || api_res?.message || "";
      const extra = deliveryText ? `\n${escapeHtml(JSON.stringify(deliveryText))}` : "";

      await sendOrEdit(ctx,
        `{emoji:SUCCESS} <b>Order Successful!</b>\n\n` +
        `Your order for <b>${escapeHtml(session.data.package_display_name || session.data.package_name)}</b> has been delivered.` +
      ...
      const purchaseProductLink = session.data.product_link ||
        (purchaseBotUsername ? `https://t.me/${purchaseBotUsername}?start=shop` : "");
      ...
      try {
        await bot.telegram.sendMessage(PROOF_CHANNEL_ID, buildMessageWithCustomEmojis(purchaseLogText), {
          parse_mode: "HTML", reply_markup: processKeyboardCustomEmojis(purchaseMarkup || {})
        });
      } catch (_) {}
    } else if (failed) {
      // G2Bulk documents FAILED game orders as automatically refunded. The
      // provider balance is therefore already restored; refund only the
      // customer's bot wallet charge here.
      await db.update_balance(user.id, charged_price);
      const err_msg = api_res?.message || api_res?.order?.message || `G2Bulk order status: ${providerStatus}`;
      await sendOrEdit(ctx, `{emoji:FAIL} <b>Order Failed</b>\n\n${String(err_msg)}\nYour funds have been refunded to your wallet.`, { reply_markup: get_main_keyboard() });
    } else {
      // Provider accepted the order but it is still processing. Do NOT refund
      // the customer because G2Bulk has already accepted/charged the order.
      await sendOrEdit(ctx, `{emoji:PENDING_PAYMENT} <b>Order Processing</b>\n\nG2Bulk accepted your order and is still delivering it.\nProvider Order: <code>${escapeHtml(providerOrderId || "pending")}</code>`, { reply_markup: get_main_keyboard() });
    }
  } catch (e) {
    // Only refund when the provider definitely did not accept the order.
    // Never refund after an accepted G2Bulk order merely because Telegram,
    // logging, or the local database raised an exception.
    if (!providerAccepted) {
      try { await db.update_balance(user.id, charged_price); } catch (refundError) {
        logger.error(`Refund failed for ${user.id}: ${refundError.message}`);
      }
    }
    logger.error(`Order flow error | providerAccepted=${providerAccepted} | ${e.stack || e.message}`);
    await sendOrEdit(ctx,
      providerAccepted
        ? `{emoji:PENDING_PAYMENT} <b>Order Accepted</b>\n\nG2Bulk accepted the order, but the bot could not finish displaying the result. Provider order: <code>${String(api_res?.order?.order_id || api_res?.order_id || "pending")}</code>`
        : `{emoji:FAIL} <b>System Error</b>\n\nFailed to place order: ${e.message}\nFunds have been refunded to your wallet.`,
      { reply_markup: get_main_keyboard() }
    );
  }
  clearUserSession(user.id);
