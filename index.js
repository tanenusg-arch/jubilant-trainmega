...
async function main() {
  ...
  bot.on("callback_query", async (ctx) => {
    try {
      ...
      const userId = ctx.from.id;
      logger.info(`[CALLBACK] user=${userId} data=${data}`);
  ...
  await bot.telegram.setWebhook(WEBHOOK_URL, {
    // Explicitly subscribe to callback_query updates. If allowed_updates is
    // omitted, Telegram keeps the previous webhook setting, which can leave
    // inline buttons completely unresponsive after a deployment.
