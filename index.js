...
class G2BulkAPIClient {
  ...
  async place_order(game_code, package_id, player_id, server_id = null, user_ip = null) {
    ...
    if (server_id) payload.server_id = String(server_id);
    logger.info(`[G2BULK] Placing game order | game=${game_code} | catalogue=${catalogue_name} | player_id=${player_id}`);
    const result = await this._request(`/games/${game_code}/order`, "POST", payload);
...
async function place_order_flow(ctx, session, db, api_client, bot, method, reference = null, verified_amount = null) {
  ...
  try {
    ...
    const success = !providerFailed && (providerAccepted || ["COMPLETED", "PROCESSING", "PENDING"].includes(providerStatus) || api_res?.message === "success" || api_res?.status === 200 || api_res?.code === 200);
    const status = providerStatus === "COMPLETED" ? "completed" : success ? "processing" : "failed";
    logger.info(`[G2BULK] Delivery decision | accepted=${success} | provider_order_id=${providerOrderId || "none"} | provider_status=${providerStatus || "unknown"}`);
