...
const BOT_LOGS_LINK = "https://t.me/Dev_LecteR";
...
function get_main_keyboard() {
  return {
    inline_keyboard: [
      ...
      [
        { text: "Developer", url: BOT_LOGS_LINK, icon_custom_emoji_id: "5251662946127336150" },
      ],
      [
...
class G2BulkAPIClient {
  ...
  async place_order(game_code, package_id, player_id, server_id = null, user_ip = null) {
    if (!game_code) throw new Error("Missing G2Bulk game code");
    if (!package_id) throw new Error("Missing G2Bulk catalogue/package");
    if (!player_id) throw new Error("Missing player ID");

    let catalogue_name = String(package_id);
    const cat = await this.get_game_catalogue(game_code);
    if (cat && cat.success && Array.isArray(cat.catalogues)) {
      const wanted = String(package_id);
      const pkg = cat.catalogues.find(p =>
        String(p.id ?? "") === wanted ||
        String(p.code ?? "") === wanted ||
        String(p.name ?? "") === wanted ||
        String(p.catalogue_name ?? "") === wanted ||
        String(p.display_name ?? "") === wanted
      );
      if (pkg) {
        catalogue_name = String(pkg.name || pkg.catalogue_name || pkg.display_name || pkg.code || package_id);
      }
    }
    ...
    logger.info(`[G2BULK] Placing game order game=${game_code} catalogue=${catalogue_name} player=${player_id} server=${server_id || "none"}`);

    const url = `${this.base_url}/games/${encodeURIComponent(game_code)}/order`;
    const headers = {
      "X-API-Key": this.api_key,
      Accept: "application/json",
      "Content-Type": "application/json",
    ...
    try {
      const response = await axios.post(url, payload, { headers, timeout: 30000, validateStatus: () => true });
      logger.info(`[G2BULK] Order response HTTP ${response.status}: ${JSON.stringify(response.data)}`);
      if (response.status >= 400) {
        return response.data || { success: false, message: `G2Bulk HTTP ${response.status}` };
      }
      return response.data;
    } catch (e) {
      logger.error(`[G2BULK] Order request failed: ${e.message}`);
...
async function verify_payment(transaction_id, method = "telebirr", expected_amount = null, options = {}) {
  ...
  try {
    ...
    const idempotencyKey = `MEGA TOPUP-${bank}-${String(transaction_id).trim()}-${uuidv4()}`;
  ...
  try {
    ...
    const success = (
      ...
      (
        ...
        api_res.success === "true" ||
        ["completed", "processing", "pending"].includes(providerStatus) ||
        api_res.message === "success" ||
        api_res.status === 200 ||
        api_res.code === 200 ||
        !!api_res.order?.order_id
      )
    );
...
async function main() {
  ...
  bot.command("terms", async (ctx) => {
    clearUserSession(ctx.from.id);
    const terms =
      `{emoji:WARNING} <b>MEGA TOPUP — RULES &amp; TERMS</b> {emoji:WARNING}\n` +
      `By using MEGA TOPUP, you automatically agree to the following Terms &amp; Conditions\n\n` +
  ...
  await bot.telegram.setMyCommands([
    { command: "start", description: "Open MEGA TOPUP main menu" },
    ...
    { command: "terms", description: "View MEGA TOPUP Rules & Terms" },
  ...
  const httpServer = http.createServer((req, res) => {
    if (req.method === "GET" && (req.url === "/" || req.url === "/health")) {
      ...
      res.end("MEGA TOPUP bot is running.");
      return;
    }
