...
class G2BulkAPIClient {
  ...
  async _request(endpoint, method = "GET", data = null, extraHeaders = {}) {
    ...
    try {
      const response = await axios({ method, url, headers, data, timeout: 30000, validateStatus: () => true });
      if (response.status >= 400) {
        logger.error(`[G2BULK] HTTP ${response.status} ${method} ${endpoint} | ${JSON.stringify(response.data)}`);
      }
      return response.data;
    } catch (e) {
      logger.error(`[G2BULK] API Error ${method} ${endpoint}: ${e.message}`);
  ...
  async get_me() {
    return await this._request("/getMe", "GET");
  }
  ...
  async place_order(game_code, package_id, player_id, server_id = null, user_ip = null, idempotency_key = null, callback_url = null) {
    let catalogue_name = package_id;
    const cat = await this.get_game_catalogue(game_code);
    const catalogues = Array.isArray(cat?.catalogues) ? cat.catalogues : [];
    const pkg = catalogues.find(p =>
      String(p?.id ?? "") === String(package_id) ||
      String(p?.name ?? p?.catalogue_name ?? p?.display_name ?? "") === String(package_id)
    );
    if (pkg) {
      catalogue_name = pkg.name ?? pkg.catalogue_name ?? pkg.display_name ?? package_id;
    }

    // G2Bulk requires the denomination name (for example "60 UC"), not the
    // catalogue numeric ID. The order endpoint also charges the G2Bulk wallet
    // when the order is accepted.
    const payload = {
      catalogue_name: String(catalogue_name),
      player_id: String(player_id),
    };
    if (server_id) payload.server_id = String(server_id);
    if (callback_url) payload.callback_url = String(callback_url);

    const headers = {};
    if (idempotency_key) headers["X-Idempotency-Key"] = String(idempotency_key);

    logger.info(`[G2BULK] PUBG order request | game=${game_code} | catalogue=${catalogue_name} | player_id=${player_id} | idempotency=${idempotency_key ? "set" : "none"}`);
    const result = await this._request(`/games/${game_code}/order`, "POST", payload, headers);
    logger.info(`[G2BULK] PUBG order response | ${JSON.stringify(result)}`);
    return result;
  }
}
...
class FirestoreDatabase {
  ...
  async get_order_by_provider_id(provider_order_id) {
    const { data, error } = await this.db.from('orders').select('*').contains('data', { provider_order_id: String(provider_order_id) }).limit(1);
    if (error) throw error;
...
async function place_order_flow(ctx, session, db, api_client, bot, method, reference = null, verified_amount = null) {
  ...
  const api_price = Number(session.data.api_price || 0);
  let orderId = session.data.pending_order_id || null;
  let walletDeducted = false;

  if (!package_id || !player_id || !charged_price) {
    throw new Error("PUBG order is missing package, player ID, or price.");
  }

  // Deduct the customer's wallet amount before the provider call, then create
  // the local order immediately so a provider timeout can be reconciled safely.
  if (method === "wallet") {
  ...
  try {
    ...
    // Stable UUID for G2Bulk idempotency. If the HTTP request times out after
    // G2Bulk accepts the order, retrying with the same key returns the same
    // provider order instead of charging the G2Bulk wallet twice.
    const idempotencyKey = uuidv4();
    const externalUrl = String(process.env.RENDER_EXTERNAL_URL || "").replace(/\/$/, "");
    const callbackUrl = externalUrl ? `${externalUrl}/g2bulk/order-status` : null;
    await db.update_order(orderId, { provider_idempotency_key: idempotencyKey, provider_status: "SUBMITTING" });

    let api_res;
    if (session.data.flow_type === "voucher") {
      api_res = await api_client.purchase_product(package_id, 1);
    } else {
      api_res = await api_client.place_order(game_code, package_id, player_id, server_id, null, idempotencyKey, callbackUrl);
    }

    const providerOrder = api_res?.order || api_res?.data?.order || null;
    const providerOrderId = providerOrder?.order_id ?? api_res?.order_id ?? api_res?.data?.order_id ?? null;
    const providerStatus = String(providerOrder?.status ?? api_res?.status ?? api_res?.data?.status ?? "").toUpperCase();
    const providerFailed = api_res?.success === false || !!api_res?.error || ["FAILED", "REFUNDED", "CANCELLED", "CANCELED"].includes(providerStatus);
    const providerAccepted = api_res?.success === true || api_res?.success === "true" || api_res?.ok === true || providerOrderId != null;
    const accepted = !providerFailed && (providerAccepted || ["PENDING", "PROCESSING", "COMPLETED"].includes(providerStatus));

    logger.info(`[G2BULK] Delivery decision | accepted=${accepted} | provider_order_id=${providerOrderId || "none"} | provider_status=${providerStatus || "unknown"}`);
    ...
    if (!accepted) {
      const errMsg = String(api_res?.message || api_res?.error || "G2Bulk rejected the order.");
      await db.update_order(orderId, { status: "failed", provider_status: providerStatus || "FAILED", provider_error: errMsg });
      if (walletDeducted || method !== "wallet") await db.update_balance(user.id, charged_price);
    ...
    // G2Bulk game orders are asynchronous: accepted orders are normally PENDING
    // and must be completed through the callback/status flow. Do not mark them
    // completed merely because the order was accepted.
    if (providerStatus !== "COMPLETED") {
      await sendOrEdit(ctx,
        `{emoji:PENDING_PAYMENT} <b>Order Processing</b>\n\n` +
        `${getProductLogName(session)} is being delivered to <b>${String(session.data.nickname || player_id)}</b>.\n\n` +
        `G2Bulk order: <code>${String(providerOrderId || "processing")}</code>\n` +
        `Please wait a moment for delivery.`,
    ...
    await db.update_order(orderId, { status: "completed", provider_status: "COMPLETED" });
    await sendOrEdit(ctx,
      `{emoji:SUCCESS} <b>Order Successful!</b>\n\n` +
      `Your order for <b>${String(session.data.package_display_name || session.data.package_name)}</b> has been delivered.` +
      `\nOrder ID: <code>${String(orderId)}</code>`,
      { reply_markup: get_main_keyboard() }
    );
    ...
    const purchaseLogText = `{emoji:STATUS}<b>New Purchase!</b>\n\nProduct: {emoji:${purchaseEmojiKey}} ${purchaseProductName}\n\n　　　{emoji:USER} By: <code>${maskPurchaseUserId(user.id)}</code>\n　　　{emoji:ORDERS}Order No: <code>${maskPurchaseOrderNo(orderId)}</code>\n\n{emoji:CALENDAR}Date : ${purchaseDate}`;
    await report_event(bot, purchaseLogText, purchaseMarkup);
  } catch (e) {
    logger.error(`[G2BULK] Order flow exception | local_order=${orderId || "none"} | ${e.stack || e.message}`);
    // Only refund when we know the provider did not accept the order. A network
    // timeout after submission is deliberately left as processing for reconciliation.
    if (orderId) await db.update_order(orderId, { status: "processing", provider_status: "UNKNOWN", provider_error: e.message || "Unknown error" }).catch(() => {});
    await sendOrEdit(ctx, `{emoji:PENDING_PAYMENT} <b>Order Processing</b>\n\nWe could not confirm the G2Bulk response yet. Your order was not marked as failed, so your payment is protected while we reconcile it.`, { reply_markup: get_main_keyboard() });
  }
  clearUserSession(user.id);
}
// --------------------------------------------
...
async function main() {
  ...
  // ---- Launch ----
  // Verify the G2Bulk API key and wallet before accepting customer orders.
  try {
    const me = await api_client.get_me();
    logger.info(`[G2BULK] Account check | success=${me?.success} | balance=${me?.balance ?? "unknown"}`);
  } catch (e) {
    logger.error(`[G2BULK] Account check failed: ${e.message}`);
  }
  ...
  const httpServer = http.createServer((req, res) => {
    ...
    if (req.method === "POST" && req.url.split("?")[0] === "/g2bulk/order-status") {
      let raw = "";
      ...
      req.on("end", async () => {
        try {
          ...
          logger.info(`[G2BULK] Webhook | order_id=${providerOrderId} | status=${status} | body=${JSON.stringify(payload)}`);
          if (providerOrderId != null) {
            const local = await db.get_order_by_provider_id(providerOrderId);
            if (local) {
              const terminalSuccess = status === "COMPLETED";
              const terminalFailure = ["FAILED", "REFUNDED", "CANCELLED", "CANCELED"].includes(status);
              await db.update_order(local.order_id, { provider_status: status, status: terminalSuccess ? "completed" : terminalFailure ? "failed" : "processing" });
              if (terminalFailure && !local.data?.provider_refunded) {
                await db.update_balance(local.telegram_id, Number(local.charged_price || 0));
                await db.update_order(local.order_id, { provider_refunded: true });
              }
              if (terminalSuccess) {
                try {
                  await bot.telegram.sendMessage(local.telegram_id, `{emoji:SUCCESS} <b>Order Delivered!</b>\n\n<b>${String(local.package_name || local.game || "PUBG Mobile")}</b> has been delivered to your account.\nOrder ID: <code>${String(local.order_id)}</code>`, { parse_mode: "HTML" });
                } catch (_) {}
              } else if (terminalFailure) {
                try {
                  await bot.telegram.sendMessage(local.telegram_id, `{emoji:FAIL} <b>Order Failed</b>\n\nG2Bulk reported <b>${status}</b>. Your payment has been refunded to your wallet.\nOrder ID: <code>${String(local.order_id)}</code>`, { parse_mode: "HTML" });
                } catch (_) {}
              }
            }
          }
          ...
          logger.error(`[G2BULK] Webhook error: ${e.message}`);
