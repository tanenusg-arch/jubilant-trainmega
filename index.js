...
// --------------------------------------------
// Default Free Fire selling prices. Admin overrides in product_overrides take precedence.
...
const DEPOSIT_METHODS = ['telebirr', 'cbe'];
const ORDER_PAYMENT_METHODS = ['telebirr', 'cbe', 'wallet'];
...
const STATE_ADMIN_FF_PRICE_INPUT = "admin_ff_price_input";
...
async function main() {
    ...
    try {
      ...
      if (ADMIN_CHAT_IDS.includes(userId)) {
        ...
        if (data === "admin_game_products") {
          ...
          if (freeFire) buttons.push([{ text: `${getBaseEmoji('FREE_FIRE')} Free Fire Prices`.trim(), callback_data: `admin_ff_prices:${normalize_game_code(freeFire)}` }]);
          else buttons.push([{ text: `${getBaseEmoji('FREE_FIRE')} Free Fire (Not found)`.trim(), callback_data: "admin_back" }]);
        ...
        if (data.startsWith("admin_ff_prices:")) {
          const code = data.slice("admin_ff_prices:".length);
          const game = (session.data.admin_games || []).find(g => normalize_game_code(g) === code);
          session.data.admin_ff_code = code;
          session.data.admin_ff_name = game ? normalize_game_name(game) : "Free Fire";
        ...
        if (data.startsWith("admin_ff_cat:")) {
          ...
          if (!code) { await sendOrEdit(ctx, `${getBaseEmoji('FAIL')} Free Fire price session expired.`, { reply_markup: { inline_keyboard: [[{ text: "🔙 Back", callback_data: "admin_game_products" }]] } }); return; }
          try {
            ...
            packages = packages.filter(pkg => {
              ...
              const isNumeric = /^\\d+$/.test(name.trim());
              if (category === "diamonds") return (name.includes("diamond") || isNumeric) && !name.includes("membership") && !name.includes("level") && !name.includes("pass") && !name.includes("booyah");
              if (category === "membership") return name.includes("membership") || name.includes("booyah") || name.includes("weekly") || name.includes("monthly");
              if (category === "levelup") return name.includes("level up") || name.includes("levelup") || /level\\s*\\d+/.test(name);
              return false;
            });
            ...
            const overrides = new Map();
            for (const ov of await db.get_all_product_overrides()) {
              if (String(ov.game_code) === String(code) && ov.price_override != null) overrides.set(String(ov.product_id || ov.id), Number(ov.price_override));
            }
            ...
            buttons.push([{ text: "🔙 Back", callback_data: `admin_ff_prices:${code}` }]);
            await sendOrEdit(ctx, `${getBaseEmoji('MONEY')} <b>${session.data.admin_ff_name} - ${category === 'diamonds' ? 'Diamonds' : category === 'membership' ? 'Membership' : 'Level Up Pass'}</b>\n\nSelect a package to change its selling price:`, { reply_markup: { inline_keyboard: buttons } });
          } catch (e) {
            await sendOrEdit(ctx, `${getBaseEmoji('FAIL')} Failed to load Free Fire packages.\n\n${e.message}`, { reply_markup: { inline_keyboard: [[{ text: "🔙 Back", callback_data: `admin_ff_prices:${code}` }]] } });
          }
          return;
        }
        ...
        if (data.startsWith("admin_ff_price:")) {
          ...
          if (!pkg) { await sendOrEdit(ctx, `${getBaseEmoji('FAIL')} Package unavailable.`); return; }
          ...
          session.data.admin_ff_price_name = clean_freefire_package_name(pkg.display_name || pkg.name || pkg.title || "Package");
          session.state = STATE_ADMIN_FF_PRICE_INPUT;
          await sendOrEdit(ctx, `Enter the selling price in ETB for <b>${session.data.admin_ff_price_name}</b>.\nEnter <code>0</code> to remove the override.`, {
  ...
  bot.on("message", async (ctx) => {
    try {
      ...
      if (session.state === STATE_ADMIN_FF_PRICE_INPUT) {
        ...
        const code = session.data.admin_ff_code;
        const category = session.data.admin_ff_category;
        session.state = STATE_ADMIN_MAIN;
        await sendOrEdit(ctx, price <= 0 ? `${getBaseEmoji('SUCCESS')} Free Fire price override removed.` : `${getBaseEmoji('SUCCESS')} Free Fire price set to <b>${Math.floor(price)} ETB</b>.`, {
          reply_markup: { inline_keyboard: [[{ text: "🔙 Manage Free Fire Prices", callback_data: `admin_ff_prices:${code}` }], [{ text: "Admin Panel", callback_data: "admin_back" }]] }
        });
        return;
      }
  ...
  // ---- Long polling launch ----
// Optional minimal HTTP health server (kept so Render web services and
// uptime monitors can still hit / or /health). Not required for polling.
...
healthServer.on("error", (e) => logger.warn(`Health server error: ${e.message}`));
healthServer.listen(PORT, "0.0.0.0", () => {
  logger.info(`Health server listening on port ${PORT}`);
});

// Remove any previously registered webhook so Telegram delivers updates
// via getUpdates (long polling) instead of POSTing to a URL.
await bot.telegram.deleteWebhook({ drop_pending_updates: false }).catch(() => {});

// Schedule the pending-payment cleanup BEFORE bot.launch() because
// bot.launch() only resolves when the bot stops.
