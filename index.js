...
if (!SUPABASE_SECRET_KEY) throw new Error("Missing SUPABASE_SECRET_KEY environment variable.");
...
const ORDER_PAYMENT_METHODS = ['telebirr', 'wallet'];
...
async function verify_payment(transaction_id, method = "telebirr", expected_amount = null, options = {}) {
  ...
  try {
    ...
    if (settlementMatch && settlementMatch.matched !== true) {
      return {
        success: false,
        error: `Payment was not received in the configured ${bank === "cbe" ? "CBE" : "Telebirr"} settlement account.`,
        server_error: false,
      };
    }
...
async function place_order_flow(ctx, session, db, api_client, bot, method, reference = null, verified_amount = null) {
  ...
  try {
    ...
    const success = isproviderDeliveryResponse || (api_res &&
    ...
    if (success) {
      ...
      // HTML. provider credentials may contain &, <, >, or other characters.
      ...
      // Delivery was normalized above for every provider product.
      ...
      const purchaseProductLink = session.data.product_link ||
        (purchaseBotUsername ? `https://t.me/${purchaseBotUsername}?start=${encodeURIComponent(purchaseProductToken)}` : "");
      ...
      // Use the same clean successful-purchase format everywhere.
      await report_event(bot, purchaseLogText, purchaseMarkup);
      try {
        await bot.telegram.sendMessage(
          PROOF_CHANNEL_ID,
          buildMessageWithCustomEmojis(purchaseLogText),
          { parse_mode: "HTML", reply_markup: processKeyboardCustomEmojis(purchaseMarkup || {}) }
        );
      } catch (_) {}
    } else {
...
async function main() {
    ...
    try {
      ...
      if (ADMIN_CHAT_IDS.includes(userId)) {
        ...
        if (data === "admin_game_products") {
          ...
          if (ff) buttons.push([{ text: `${getBaseEmoji('FREE_FIRE')} Free Fire Middle East Prices`.trim(), callback_data: `admin_ff_select:${normalize_game_code(ff)}` }]);
        ...
        // ---- Free Fire Middle East price management ----
        if (data.startsWith("admin_ff_select:")) {
          const code = data.slice("admin_ff_select:".length);
          session.data.admin_ff_code = code;
          const game = (session.data.admin_games || []).find(g => normalize_game_code(g) === code);
          session.data.admin_ff_name = game ? normalize_game_name(game) : "Free Fire Middle East";
          await sendOrEdit(ctx, `${getBaseEmoji('FREE_FIRE')} <b>${session.data.admin_ff_name} Prices</b>\n\nSelect a category:`, {
            reply_markup: { inline_keyboard: [
              [{ text: `${getBaseEmoji('FF_DIAMONDS')} Diamonds`, callback_data: "admin_ff_cat:diamonds" }],
              [{ text: `${getBaseEmoji('FF_MEMBERSHIP')} Membership`, callback_data: "admin_ff_cat:membership" }],
              [{ text: `${getBaseEmoji('FF_LEVELUP')} Level Up Pass`, callback_data: "admin_ff_cat:levelup" }],
        ...
        if (data.startsWith("admin_ff_cat:")) {
          const category = data.slice("admin_ff_cat:".length);
          const code = session.data.admin_ff_code;
          if (!code) { await sendOrEdit(ctx, "Free Fire price session expired."); return; }
          try {
            const res = await api_client.get_game_catalogue(code);
            if (!res || !res.success) throw new Error((res && res.message) || "No catalogue returned");
            let packages = res.catalogues || res.data || [];
            packages = packages.filter(pkg => {
              const name = String(pkg.display_name || pkg.name || pkg.title || "").toLowerCase();
              const numeric = /^\d+[km]?$/i.test(name.trim());
              if (category === "diamonds") {
                return (numeric || name.includes("diamond")) && !name.includes("membership") && !name.includes("level") && !name.includes("pass") && !name.includes("booyah");
              }
              if (category === "membership") return name.includes("membership") || name.includes("booyah") || name.includes("weekly") || name.includes("monthly");
              if (category === "levelup") return name.includes("level up") || name.includes("levelup") || name.includes("level");
              return false;
            });
            if (!packages.length) throw new Error(`No ${category} packages found.`);
            packages = sort_game_packages(code, packages);
            session.data.admin_ff_packages = packages;
            session.data.admin_ff_category = category;
            const overrides = new Map();
            for (const ov of await db.get_all_product_overrides()) {
              if (String(ov.game_code) === String(code) && ov.price_override != null) {
                overrides.set(String(ov.product_id || ov.id), Number(ov.price_override));
              }
            }
            const markup = await db.get_game_markup(code);
            const buttons = packages.map((pkg, idx) => {
              const id = String(pkg.id || pkg.code || pkg.name || idx);
              const raw = clean_freefire_package_name(pkg.display_name || pkg.name || pkg.title || id);
              const base = Number(pkg.unit_price ?? pkg.price ?? pkg.amount ?? pkg.cost ?? 0);
              const fixed = getFixedFFPrice(category, raw);
              const price = overrides.has(id) ? overrides.get(id) : (fixed !== null ? fixed : api_price_to_birr(base, markup));
              return [{ text: `${raw} - ${Math.floor(price)} ETB`, callback_data: `admin_ff_price:${idx}` }];
            });
            buttons.push([{ text: "🔙 Back", callback_data: `admin_ff_select:${code}` }]);
            await sendOrEdit(ctx, `${getBaseEmoji('MONEY')} <b>${session.data.admin_ff_name} - ${category[0].toUpperCase()+category.slice(1)}</b>\n\nSelect a package to change its selling price:`, {
              reply_markup: { inline_keyboard: buttons }
            });
          } catch (e) {
            logger.error(`Failed to load FF admin prices: ${e.message}`);
            await sendOrEdit(ctx, `{emoji:FAIL} Failed to load Free Fire packages.\n\n${e.message}`, {
              reply_markup: { inline_keyboard: [[{ text: "🔙 Back", callback_data: `admin_ff_select:${code}` }]] }
            });
          }
          return;
        }

        if (data.startsWith("admin_ff_price:")) {
          const idx = parseInt(data.split(":")[1], 10);
          const pkg = (session.data.admin_ff_packages || [])[idx];
          if (!pkg) { await sendOrEdit(ctx, `{emoji:FAIL} Package unavailable.`); return; }
          session.data.admin_game_code = session.data.admin_ff_code;
          session.data.admin_game_price_product_id = String(pkg.id || pkg.code || pkg.name || idx);
          session.data.admin_game_price_name = clean_freefire_package_name(pkg.display_name || pkg.name || pkg.title || "Package");
          ...
          await sendOrEdit(ctx, `Enter the selling price in ETB for <b>${session.data.admin_game_price_name}</b>.\nEnter <code>0</code> to remove the override.`, {
            reply_markup: { inline_keyboard: [[{ text: "🔙 Back", callback_data: `admin_ff_cat:${session.data.admin_ff_category}` }]] }
          });
          return;
        }
  ...
  bot.on("message", async (ctx) => {
    try {
      ...
      if (session.state === STATE_ADMIN_GAME_PRICE_INPUT) {
        ...
        const wasFF = !!session.data.admin_ff_category;
        session.state = STATE_ADMIN_MAIN;
        if (wasFF) {
          await sendOrEdit(ctx, price <= 0 ? `{emoji:SUCCESS} Free Fire package price override removed.` : `{emoji:SUCCESS} Free Fire package price set to <b>${Math.floor(price)} ETB</b>.`, {
            reply_markup: { inline_keyboard: [[{ text: "🔙 Back to Free Fire Prices", callback_data: `admin_ff_cat:${session.data.admin_ff_category}` }]] }
          });
        } else {
      ...
      if (session.state === STATE_PAYMENT_TXN_ID || session.state === STATE_PAYMENT_CBE_SUFFIX || session.state === STATE_PAYMENT__SUFFIX) {
        ...
        if (["telebirr", "cbe"].includes(method)) {
          result = await verify_payment(reference, method, charged_price, method === "cbe" ? { accountSuffix } : {});
        } else {
      ...
      if (session.state === STATE_DEPOSIT_TRANSACTION_ID || session.state === STATE_DEPOSIT_CBE_SUFFIX) {
        const method = session.data.dep_method || "telebirr";
        let reference;
        let accountSuffix = null;
        ...
        let result;
        if (["telebirr", "cbe"].includes(method)) {
          result = await verify_payment(reference, method, intended, { exactAmount: true, ...(method === "cbe" ? { accountSuffix } : {}) });
        } else {
  ...
  // Render provides the public HTTPS URL in RENDER_EXTERNAL_URL.
  // We use Telegraf's webhook callback directly so Telegram POSTs updates
  // to this service instead of using getUpdates/long polling.
  const PORT = Number(process.env.PORT || 10000);
  const RENDER_EXTERNAL_URL = String(process.env.RENDER_EXTERNAL_URL || "").replace(/\/$/, "");
  const WEBHOOK_PATH = "/telegram-webhook";
  if (!RENDER_EXTERNAL_URL) {
    throw new Error("Missing RENDER_EXTERNAL_URL. Deploy this bot as a Render Web Service with webhook mode enabled.");
  }
  ...
  const server = http.createServer(async (req, res) => {
    try {
      ...
      logger.error(`Webhook request error: ${e.stack || e.message || e}`);
      if (!res.headersSent) {
  ...
  server.on("error", (e) => logger.error(`HTTP server error: ${e.message}`));

  await new Promise((resolve, reject) => {
    server.once("error", reject);
    server.listen(PORT, "0.0.0.0", () => {
      server.removeListener("error", reject);
      logger.info(`Render webhook server listening on port ${PORT}`);
      resolve();
    });
  });

  // Register the exact public webhook URL. drop_pending_updates=false keeps
  // updates Telegram already has queued for the bot.
  await bot.telegram.setWebhook(webhookUrl, {
    drop_pending_updates: false
  });

  const webhookInfo = await bot.telegram.getWebhookInfo();
  logger.info(`[WEBHOOK] URL=${webhookInfo.url || ""} pending=${webhookInfo.pending_update_count || 0}`);
  if (webhookInfo.url !== webhookUrl) {
    throw new Error(`Telegram webhook registration mismatch. Expected ${webhookUrl}, got ${webhookInfo.url || "<empty>"}`);
  }

  // Schedule pending-payment cleanup after the HTTP server is ready.
  ...
  logger.info(`[WEBHOOK] Telegram webhook active at ${webhookUrl}`);
}
