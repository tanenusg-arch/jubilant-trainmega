...
const SUPABASE_SERVICE_ROLE_KEY = "sb_secret_btNh6_Jv6oTbHk7EnbPH5Q_dyOp6weC";
...
const BOT_LOGS_LINK = "https://t.me/yabtopcghtdfuffgupp_group";
const CHANNEL_LINK = "https://t.me/Alafrica_shop";
const TELEBIRR_PHONE = "0972271794"; // updated
...
const EXPECTED_RECEIVER_NAME = "WORKE BEKELE TESSEMA"; // updated
const CACHE_TTL = 600;
...
const telegramWebhookHandler = bot.webhookCallback(WEBHOOK_PATH);
...
// Configure Telegram webhook for Render. Set WEBHOOK_URL to your public
// Render URL, or let the code use RENDER_EXTERNAL_URL automatically.
if (!WEBHOOK_URL) {
  throw new Error("WEBHOOK_URL or RENDER_EXTERNAL_URL must be set for webhook mode");
}
const publicWebhookUrl = `${WEBHOOK_URL.replace(/\/$/, "")}${WEBHOOK_PATH}`;
await bot.telegram.setWebhook(publicWebhookUrl);
logger.info(`Telegram webhook configured: ${publicWebhookUrl}`);

// Schedule the pending-payment cleanup before keeping the HTTP server alive.
...
logger.info("Telegram bot running in Render webhook mode...");
}
